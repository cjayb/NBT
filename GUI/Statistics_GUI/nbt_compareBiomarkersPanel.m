...
function StatObj = nbt_compareBiomarkersPanel(StatObj, StudyObj)
    ...
    ListBiom2 = uicontrol(hp2,'Units', 'pixels','style','listbox','Max',1,'Units', 'pixels','Position',[5 5 290 130],'fontsize',8,'String',biomarkerNames);
    % tests
    hp3 = uipanel(StatSelection,'Title','Select Biomarker to compare','FontSize',8,'Units','pixels','Position',[5 410 300 150]);
    ListBiom1 = uicontrol(hp3,'Units', 'pixels','style','listbox','Max',1,'Units', 'pixels','Position',[5 5 290 130],'fontsize',8,'String',biomarkerNames);
    % select Group
    for i = 1:length(StudyObj.groups)
        gname = StudyObj.groups{i}.groupName;
    ...
    ViewGraphsButton = uicontrol(StatSelection,'Style','pushbutton','String',{'View Splitting', 'Graphs'},'Position',[425 180 100 50],'fontsize',8,'callback',{@nbt_splitPlotGroups,ListSplit,ListSplitValue,ListBiom1, ListBiom2, ListRegion, ListGroup, StatObj});

    RunButton = uicontrol(StatSelection,'Style','pushbutton','String','Run Test','Position',[250 50 100 50],'fontsize',8,'callback',{@nbt_compareBiomarkersGetSettings,ListBiom1, ListBiom2, ListRegion, ListGroup,ListTest,ListDisplay,ListSplit,ListSplitValue, StatObj});
