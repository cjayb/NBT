...
global NBTstudy
    ...
    uimenu(hh,'label','Plot topoplot','callback',{@plot_test2Groups,StatObj});
    %uimenu(hh2,'label','Plot boxplot','callback',{@plot_subj_vs_subj,x,stat_results,regs,Group1,Group2,regs_or_chans_name});
    uicontrol(h2, 'Style', 'pushbutton', 'string', 'Plot topoplots of all biomarkers', 'position', [20 400 200 20],'callback',{@plot_test2GroupsAll,StatObj});
    close(h1)
 ...
 function plot_test2Groups(d1,d2,StatObj)
     ...
     function plot_test2GroupsAll(d1,d2,StatObj)
        ...
        hold on;
            nbt_plot_2conditions_topoAll(StatObj);
    end